...
func (t *Tree) parse() {
	...
	for t.peek().typ != itemEOF {
		debug.TraceF("peek item type %s %s", itemTypeName[t.peek().typ], t.peek().String())
...
func (t *Tree) action() (n Node) {
	...
	case itemTemplate:
		return t.reservedKeyword(token, "action")
	case itemWith:
...
decls:
	if v := t.peekNonSpace(); v.typ == itemVariable {
		debug.TraceF("decls token type %s %s", itemTypeName[token.typ], token.String())
		t.next()
	...
	for {
		token := t.nextNonSpace()
		debug.TraceF("for loop token type %s %s", itemTypeName[token.typ], token.String())
		switch token.typ {
		case end:
...
func (t *Tree) parseControl(allowElseIf bool, context string) (pos Pos, line int, pipe *PipeNode, list, elseList *ListNode) {
	debug.TraceF("allowElseIf %v context %s", allowElseIf, context)
...
// unexpected complains about the token and terminates processing.
func (t *Tree) reservedKeyword(token item, context string) Node {
	if token.typ == itemError {
		extra := ""
		if t.actionLine != 0 && t.actionLine != token.line {
			extra = fmt.Sprintf(" in action started at %s:%d", t.ParseName, t.actionLine)
	...
	t.errorf("reserved keyword %s in line %d", token, token.line)
	return t.newDummy(token.pos, token.line)
}
...
func (t *Tree) operand() Node {
	...
	debug.TraceF("node %s", node.String())
